...
- The estimator decomposes the covariance surface into nonparametric marginal scale functions and a structured correlation surface.

- The correlation component is fit over a menu of interpretable positive-definite
  families and then averaged using data-driven weights.

- The resulting estimator is computationally attractive, retains flexibility
  where the data are most informative, and provides a direct bridge between
  fully parametric and fully nonparametric sparse FDA covariance estimation.

- Data-driven tuning parameter selection is used throughout.

- An empirical CD4 count application is presented.
...
- Classical data: $Y_i$ is a finite-dimensional draw from a random vector $Y$:
  $$
  Y_i \in \mathbb R^d .
  $$

- Functional data: $X_i$ is an infinite-dimensional draw from a random function
  $X(\cdot)$, indexed by a continuum $t \in \mathcal T$:
  $$
...
::: {.notes}
Begin with the sample element before proceeding.
:::
...
The library is deliberately heterogeneous; model-averaging is over shapes, not just
