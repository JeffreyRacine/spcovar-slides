...
local({
  ...
  x <- stats::runif(n_curves)
  y <- mean_function(x) + stats::rnorm(n_curves, sd = sigma)
...
::: {.notes}
The left panel contains finite-dimensional observations. Each colored line in
the right panel is one random sample element: an entire latent trajectory.
This is a different sampling object, not simply a regression curve fitted to
the left-hand points. This faithfully uses the historical example's
$N=25$, $M=1$, 100-point grid, $\sigma=0.5$, $\tau=1$, and $L=1$
parameterization; each trajectory includes an independent $N(0,1)$
curve-specific random intercept.
:::

## Functional Data

- Functional data analysis (FDA) is the statistical analysis of samples of
...
- Let $X=\{X(t):t\in[0,1]\}$ be a second-order stochastic process with
  continuous trajectories. Its mean and covariance functions are
  $$
  \mu(t)=\mathbb E\{X(t)\},
  \qquad
...
function space. Here the random functions $X_i(\cdot)$ are latent; the data are
the discrete pairs $(Y_{ij},T_{ij})$.
:::
