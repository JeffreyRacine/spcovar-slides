...
format:
  revealjs:
    ...
    footer: "Sparse covariance FDA | V. Patilea and J. S. Racine"
...
is learned from irregularly placed within-subject pairs.
...
$$
\widehat\mu(t)
=\sum_{k=0}^{K}\widehat b_k \phi_k(t),
$$

where the basis is selected and truncated from the pooled sample.

The series step is one-dimensional and computationally inexpensive.

## Variance Estimation

After estimating the mean, latent marginal variance is estimated from squared
centered responses:

$$
V_{ij}
=\{Y_{ij}-\widehat\mu(T_{ij})\}^2.
$$
...
$$
E(V_{ij}\mid T_{ij}=t)
\approx \sigma^2(t)+\sigma_\varepsilon^2 .
$$

Subtracting a noise-variance estimate gives the latent variance estimate.
...
Average the $k_n$ within-curve pairs with the smallest time gaps
$|T_{ij}-T_{ik}|$:

$$
\widehat\sigma_\varepsilon^2
=\frac{1}{k_n}\sum_{(i,j,k)\in\mathcal N_{k_n}}D_{ijk}.
$$

The operational rule is deliberately simple:

$$
k_n \asymp \log(n_{\mathrm{pooled}}).
$$
...
In finite samples, boundary correction can increase integrated RMSE even while
...
## Table 1-4 Style Evidence

The first block compares the proposed estimator with parametric and
nonparametric competitors across core correlation DGPs.

The key object is covariance RMSE:

$$
\operatorname{RMSE}
= \sqrt{1000\,\operatorname{MSE}}.
$$

Relative RMSE is reported against the relevant baseline.

## What The Evidence Is Designed To Show

The semiparametric estimator should be strong when:

1. marginal location and scale are not parametric;
2. the correlation surface resembles one or more candidate families;
3. direct tensor smoothing has too little information.

The benchmark is not designed to make one estimator win everywhere.
...
- how do RMSE and rankings change across DGPs?

## Peer Comparison Ratios: Bias Corrected
...
## Peer Comparison Ratios: Not Bias Corrected
...
The $\widehat\alpha$ weights are diagnostic, not just computational detail.
...
## Averaging Weights: Proposed

Average $\widehat\alpha$ weights for the proposed estimator. The first tab
...
## Averaging Weights: Family Omitted
...
## Reading The Simulation Tables

The table sequence mirrors the manuscript:

1. overall RMSE and relative RMSE;
2. DGP-specific RMSE;
3. model-averaging weights;
4. leave-family-out weights;
5. peer comparison and rankings.
...
Then fit candidate correlations to $\widehat U_{ijk}$.
