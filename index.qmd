...
|---|---|---|---|
| SP | proposed `spcovar` | boundary-corrected margins, parametric correlation library, convex averaging | low-dimensional bivariate structure |
...
- `SP`: `spcovar(..., mean_basis = "cosine_bc_gs2", var_basis = "cosine_bc_gs2", mean_lag_method = "smoothed", var_lag_method = "smoothed")`.
- `SP-OMIT`: same call with `fit_families = campaign_drop_own_family(dgp_key)`.
- `NP`: `npcovar(..., nomad = TRUE, covariance_basis = "cosine_bc_gs2", standardize = TRUE)`.
